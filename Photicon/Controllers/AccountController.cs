...
namespace Photicon.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            var result = await SignInManager.PasswordSignInAsync(model.UserNameIdentity, model.Password, model.RememberMe, shouldLockout: false);
            switch (result)
            {
                case SignInStatus.Success:
                   var userId = db.Users
                        .Where(m => m.UserName == model.UserNameIdentity)
                        .Select(m => m.Id)
                        .SingleOrDefault();
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    return RedirectToAction("MainPage", "Home", new {Id=user.Id});
                }
                AddErrors(result);
            }
        ...
        public async Task<ActionResult> ExternalLoginCallback(string returnUrl)
        {
            ...
            user = db.Users.Where(m => m.Email == loginInfo.Email).Select(m => m).SingleOrDefault();
            user.PicturesList = db.Pictures.Where(m => m.User == user).Select(m => m).ToList();
            if (loginInfo == null)
            {
